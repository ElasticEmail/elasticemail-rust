...
pub async fn campaigns_by_name_pause_put(configuration: &configuration::Configuration, name: &str) -> Result<(), Error<CampaignsByNamePausePutError>> {
    ...
    let local_var_uri_str = format!("{}/campaigns/{name}/pause", local_var_configuration.base_path, name=crate::apis::urlencode(name));
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<CampaignsByNamePausePutError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
