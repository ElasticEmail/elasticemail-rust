...
#[tokio::main]
async fn main() {
	let recipient = ElasticEmail::models::EmailRecipient{
		email: "<to address>".to_string(),
		fields: None
	};
	let body_content_type = ElasticEmail::models::BodyContentType::Html;
	let content = ElasticEmail::models::BodyPart{
	...
	convec.push(content);
	let email = ElasticEmail::models::EmailContent{
		from: Some(String::from("<from address>")),
		subject: Some(String::from("Test")),
		body: Some(convec),
		..Default::default()
	};
	let mut recvec = Vec::new();
	recvec.push(recipient);
	let email_box = Box::new(email);
	let data = ElasticEmail::models::EmailMessageData{
		recipients: recvec,
		content: Some(email_box),
		options: None
	};
	let api_key = ElasticEmail::apis::configuration::ApiKey{
		key: "<your API key>".to_string(),
		prefix: None
	};
	let config = ElasticEmail::apis::configuration::Configuration{
		api_key: Some(api_key),
		user_agent: None,
		..Default::default()
	};
	let resp = ElasticEmail::apis::emails_api::emails_post(&config, data);
	match resp.await {
